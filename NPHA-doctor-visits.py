...
data = pd.read_csv('C:\\Users\\李昌峻\\Desktop\\NPHA-doctor-visits.csv',names=column_names)
# 引入名字列表
...
# 有标签：
for i in name:
    plt.scatter(data[x_axis][data['class']==i],data[y_axis][data['class']==i],label=name)
...
# 参数设置
K = 2 # 设置聚类数量（单独的算法，iris中已知标签数目，定义位3）
max_train = 50000 # 设置最大迭代次数

# 训练k-means
cluster, centroids, K = KMeans_train(data, K, max_train)
color=['r','b','g','black','orange'] # 绘制不同簇的颜色
for i in range(K):
